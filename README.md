...
- **趣味交互体验**：对话界面引入像素风小 IP，实现眼球跟随鼠标移动的趣味动画交互。
- **完善的消息中心**：支持管理员审核提醒、用户审核结果通知及智能体下架提醒，具备一键已读和列表刷新功能。
- **RAG 增强 (New!)**：内置 **理工助手Pro** 高级智能体，基于 RAG (检索增强生成) 技术，支持上传 PDF 知识库，能够准确回答特定领域的私有知识问题。
- **双路召回与混合检索 (New!)**：支持同时开启 **联网搜索** 和 **知识库检索**，AI 能够智能融合互联网实时信息与本地私有文档，提供全方位的回答。
- **智能体全生命周期管理**：
...
将 PDF 文件放入 `documents/source` 目录，然后运行：
